...
int nil_main(int argc,char **argv) {
	...
	if (strcasecmp(mode,"server") == 0) {
		...
		run_bot(config);
	} else if (strcasecmp(mode,"serverclient") == 0) {
#ifdef _WIN32
		std::vector<const char*> args(argv,argv+argc);
		args.push_back("mode=client");
		args.push_back(NULL);

		int pi = _spawnv(_P_NOWAIT, argv[0], (char * const *)&args[0]);
		run_server(config);
		TerminateProcess((HANDLE)pi, 0);
#else
		logmsg(lt_message,"Going to fork off a client and run a server...");
		//fork off a client before running the server
		pid_t pid = fork();
		if (pid == 0)
			run_client(config);
		else {
...
#endif
	} else if (strcasecmp(mode,"clientserver") == 0) {
#ifdef _WIN32
		std::vector<const char*> args(argv,argv+argc);
		args.push_back("mode=server");
		args.push_back(NULL);
		...
		TerminateProcess((HANDLE)pi, 0);
#else
		logmsg(lt_message,"Going to fork off a server and run a client...");
...
#endif
	} else if (strcasecmp(mode,"cleanup") == 0) {
		logmsg(lt_debug,"This mode is obsolete");
	} else if (strcasecmp(mode,"savelump") == 0) {
		...
		int lump_size= loader.save_lump(buffer, bs);
		if (lump_size > 0) {
			logmsg(lt_debug, "Saving %i bytes to %s", lump_size, expanded_filename);
			FILE *file = fopen(expanded_filename, "wb");
			fwrite(buffer, lump_size, 1, file);
			fclose(file);
		}
		free(buffer);
	} else if(strcasecmp(mode,"viewfile")==0) {
		view_rgbf(config);	  
	} else {
		logmsg(lt_fatal,"This cannot possibly happen, mode was: '%s'",mode);
		return 3;
	}
	return 0;
}
